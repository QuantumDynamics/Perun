...
static uint8_t NRFGetStatus(void)
{
	...
	SPIExchangeData(&NRF_SPI, command, result, 2);
...
void NRFFlushTX(void)
{
	...
	SPIExchangeData(&NRF_SPI, &command, &result, 1);
}
...
static void NRFReadRXFifo(uint8_t *outBuf)
{
	...
	SPIExchangeData(&NRF_SPI, &command, inBuf, NRF_FIFO_BYTES + 1);
...
static void NRFWriteTXFifo(uint8_t *inBuf)
{
	...
	SPIExchangeData(&NRF_SPI, command, bogus, NRF_FIFO_BYTES);
}
...
static void NRFPrintAddrs(void)
{
	...
	SPIExchangeData(&NRF_SPI, txbuf, rxbuf, 6);
